...
function generateReasoningPath(problem: string, index: number): ConsistencySample {
  ...
  return {
    path: index + 1,
    reasoning: `Path ${index + 1} (${approach}): Analyzing "${problem.substring(0, 80)}${
      problem.length > 80 ? '...' : ''
    }" through ${approach.replace(/-/g, ' ')}.`,
    conclusion: `Conclusion from path ${
      index + 1
    }: The solution involves systematic analysis with ${approach.replace(/-/g, ' ')} approach.`,
...
const debateTool: Tool = {
  ...
  execute: async (args: Record<string, unknown>, _ctx: PluginContext): Promise<ToolCallResult> => {
    ...
    try {
      ...
      const winner: DebateResult['winner'] = proScore > conScore
        ? 'pro'
        : conScore > proScore
        ? 'con'
        : 'tie';
    ...
    try {
      if (!args.original_output || typeof args.original_output !== 'string') {
        return {
          ...
          success: false,
          output: '',
          error: 'original_output must be a non-empty string',
          durationMs: Date.now() - start,
        };
      }
      if (!args.reflection_results || typeof args.reflection_results !== 'string') {
        return {
          ...
          success: false,
          output: '',
          error: 'reflection_results must be a non-empty string',
          durationMs: Date.now() - start,
        };
      }
      ...
      try {
        ...
        return {
          ...
          success: false,
          output: '',
          error: 'reflection_results must be valid JSON',
          durationMs: Date.now() - start,
        };
      }
      ...
      if (reflectionData.consensusLevel === 'low') {
        suggestions.push({
          ...
          suggestion:
            'Multiple reasoning paths diverged. Consider breaking the problem into smaller sub-problems.',
          priority: 'high',
        });
      }
      ...
      if (reflectionData.score !== undefined && (reflectionData.score as number) < 80) {
        suggestions.push({
          ...
          suggestion:
            `Verification score is ${reflectionData.score}/100. Add more explicit checks for edge cases and error handling.`,
          priority: 'high',
        });
      }
      ...
      if (reflectionData.winner === 'con') {
        suggestions.push({
          category: 'logic',
          suggestion:
            'Counter-arguments prevailed in debate. Address the identified weaknesses before finalizing.',
          priority: 'high',
        });
      }
      ...
      if (originalOutput.length < 50) {
        suggestions.push({
          ...
          suggestion:
            'Output is brief. Add more detail, examples, or justification to improve quality.',
          priority: 'medium',
        });
      }
      ...
      return {
        ...
        success: true,
        output: JSON.stringify({
          originalOutput: originalOutput.substring(0, 200) +
            (originalOutput.length > 200 ? '...' : ''),
          suggestions,
